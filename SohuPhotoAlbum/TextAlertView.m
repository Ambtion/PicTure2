...
- (id)initWithDelegate:(id<TextAlertViewDelegate>)delegate name:(NSString *)name
{
    ...
    if (self) {
        ...
        UILabel * label = [[UILabel alloc] initWithFrame:CGRectMake(0, 20, 240, 15)];
        ...
        UIImageView * bg_view = [[UIImageView alloc] initWithFrame:CGRectMake((240 - 192)/2.f, 49, 192, 35)];
...
- (void)cancelClicked
{
    if ([_delegate respondsToSelector:@selector(textAlertView:cancelClicked:)]) {
        [_delegate textAlertView:self cancelClicked:_renameField];
    }
...
- (void)okClicked
{
    ...
    if ([EmojiUnit stringContainsEmoji:_renameField.text]) {
        PopAlertView * tip = [[PopAlertView alloc] initWithTitle:@"提示信息" message:@"专辑名称不能包含特殊字符或表情" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil];
        [tip show];
        return;
    }
    if ([_delegate respondsToSelector:@selector(textAlertView:OKClicked:)]) {
        [_delegate textAlertView:self OKClicked:_renameField];
    }
    if ([_renameField isFirstResponder]) {
        [_renameField resignFirstResponder];
    }else{
